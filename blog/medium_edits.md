...
The **▶ Run this section on Colab** lines are the tutorial links (one notebook per section, in
`notebooks/`). On Medium, paste the link on its own line — it renders as a link card — or
attach the badge image `https://colab.research.google.com/assets/colab-badge.svg` linked to it.
The links only work after the repo is pushed and the `v1.0-artifacts` release exists.
...
> ▶ **Run this section on Colab:** [tutorial_01_data_and_baselines](https://colab.research.google.com/github/juanmigutierrez/generative-recommendation-engine/blob/main/notebooks/tutorial_01_data_and_baselines.ipynb) — download the data, build the split, run the baselines
>
...
> into three digits.
>
> ▶ **Run this section on Colab:** [tutorial_02_semantic_ids](https://colab.research.google.com/github/juanmigutierrez/generative-recommendation-engine/blob/main/notebooks/tutorial_02_semantic_ids.ipynb) — embed a 5K-item subset, train the RQ-VAE, look at the clusters
...
> later.
>
> ▶ **Run this section on Colab:** [tutorial_03_generative_retrieval](https://colab.research.google.com/github/juanmigutierrez/generative-recommendation-engine/blob/main/notebooks/tutorial_03_generative_retrieval.ipynb) — train on a 20K-user subset for five minutes, run constrained beam
...
> mattered least.
>
> ▶ **Run this section on Colab:** [tutorial_04_ranking](https://colab.research.google.com/github/juanmigutierrez/generative-recommendation-engine/blob/main/notebooks/tutorial_04_ranking.ipynb) — build the features, reproduce the injection bug in one cell and fix
> it in the next
...
>
> ▶ **Run this section on Colab:** [tutorial_05_evaluation](https://colab.research.google.com/github/juanmigutierrez/generative-recommendation-engine/blob/main/notebooks/tutorial_05_evaluation.ipynb) — load the trained checkpoints and reproduce this table
...
## 12. Result images for the post (blog/Images/)

Medium doesn't render markdown tables well, so each table is also a PNG:

- `table_time_split.png` — section "What each piece actually bought"
- `table_leave_one_out.png` — section "Measuring it the way the papers do"
- `table_reranker.png` — Ranking section, after the fix paragraph
- `recall_two_protocols.png` — the one-figure summary of the whole point (recall@10 under both
  protocols); works well right before "Measuring it the way the papers do"
